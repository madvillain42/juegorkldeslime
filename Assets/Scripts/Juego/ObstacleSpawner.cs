...
public class ObstacleSpawner : MonoBehaviour
{
    ...
    [Header("NUEVO: Configuración de Meta (Jefe)")]
    [SerializeField] private float alturaBoss = 700f; // Altura donde aparecerá la niebla
    [SerializeField] private GameObject nieblaPrefab; // Arrastra aquí tu prefab de la Niebla
    ...
    void Update()
    {
        ...
        // VERIFICACIÓN DE META: Si el jugador llega a la altura del jefe, spawneamos la niebla y nos apagamos
        if (player != null && player.position.y >= alturaBoss - spawnDistanceAhead && !nieblaSpawneada)
        {
            ...
            {
                // Aparece en el centro exacto (X=0) y a la altura designada
                Instantiate(nieblaPrefab, new Vector3(0f, alturaBoss, 0f), Quaternion.identity);
            }

            Debug.Log("[Spawner] ¡Meta alcanzada! Spawneando niebla y desactivando generación de obstáculos.");
            
            // Apagamos este componente para dejar la pantalla limpia para la transición
            this.enabled = false; 
            return;
        }
